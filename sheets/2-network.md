...
| 도구 | 용도 |
|---|---|
| ping | ICMP Echo로 도달성 확인 |
| traceroute | TTL을 1씩 늘려 경로 추적 |
| tcpdump | CLI 패킷 캡처 |
| **Wireshark** | 네트워크 패킷을 캡처해서 내용을 분석하는 도구 (GUI) |
...
### 탐지 vs 역추적 — 넷 다 호스트 / 네트워크로 갈린다

> 위치 축은 같고 **목적이 다르다**. IDS는 **지금 일어나는 침입을 탐지**하고, 역추적은 **공격자가 어디서 왔는지 추적**한다.

| 구분 | HIDS | NIDS |
|---|---|---|
| 관찰 대상 | 로그·파일·프로세스 | 패킷·트래픽 |
| 설치 위치 | 개별 호스트 | 네트워크 구간(미러링) |
| 장점 | 호스트 내부 행위까지 상세 분석 | 여러 호스트를 한 번에 감시 |
| 단점 | **호스트마다 설치** 필요 | **암호화된 내용은 분석 어려움** |

| 구분 | 호스트 기반 역추적 | 네트워크 기반 역추적 |
|---|---|---|
| 관찰 대상 | 경유 호스트의 연결 정보·로그 | 패킷·연결 정보 |
| 관찰 위치 | 경유 호스트 | 네트워크 |
| 장점 | 경유 연결관계를 확인할 수 있다 | **호스트 협조 없이** 추적한다 |
| 단점 | 경유 호스트의 **협조·정보 필요** | 추적 **인프라 필요** |

- 역추적 시스템의 두 기법: **TCP 연결 역추적** / **IP 패킷 역추적**

**"통합" 계열은 통합 대상이 서로 다르다 — 계층 아님, 병렬**

| 계열 | 통합 대상 |
|---|---|
| **UTM** | 여러 보안 **기능**을 하나의 장비로 통합(방화벽+IPS+AV+VPN). 관리 편의 ↔ 단일 장애점(SPOF) |
...
- **암호화 트래픽은 NIDS가 못 본다.** 호스트에서 복호화된 뒤를 보는 HIDS와 갈리는 지점이다.
- IDS(탐지)와 역추적(추적)은 **목적이 다르다.** 둘 다 호스트/네트워크로 나뉘어 보기에서 섞어 낸다.
...
> 실제로 틀린 문항에 남긴 대조 메모만 모았다. **반복 오답 상위가 전부 이 과목**(주소 체계·VLAN·무선 표준)에 몰려 있다.

### 주소 체계 — 가장 많이 틀린 구간

| 항목 | 값 |
|---|---|
| **IP 클래스** | A 대규모 / B 중간 / C 소규모 / **D 멀티캐스트 / E 실험·예약** |
| **사설 IPv4** | **10** / **172.16 ~ 172.31** / **192.168** |
| **IPv6 접두사** | **FE 링크 로컬 / FC 내부(유니크 로컬) / FF 멀티캐스트 / 2 글로벌** |

### 장비·프로토콜

| 항목 | 구분 |
|---|---|
| **Switch / Router** | 같은 네트워크 내 전달 / 서로 다른 네트워크 간 전달 |
| **NetBIOS** | 근거리 네트워크용. **137/UDP 이름 등록·조회, 138/UDP 비연결 데이터그램, 139/TCP 연결형 세션** |
| **SNMP** | 장비의 상태·성능·구성 정보 수집. 일반적으로 **UDP** |
| SNMPv3 보안 수준 | **noAuthNoPriv**(인증✗ 암호✗) / **authNoPriv**(인증○ 암호✗) / **authPriv**(인증·무결성·기밀성) |

### 구성·접근통제

| 항목 | 내용 |
|---|---|
| **VMPS** | MAC 주소 기반으로 VLAN을 **동적 할당** |
| **DTP** | 스위치 간 **Trunk 링크를 자동 협상** |
| **Trunk** | 물리 링크 하나로 **여러 VLAN** 트래픽을 함께 전달 |
| **802.1X** | **포트 기반** 접근제어·인증 |
| **802.11i** | 무선 **보안 표준**, WPA2의 기반 |
| **Ad-hoc / Rogue AP** | AP 없이 단말끼리 직접 통신 / **비인가 AP** |

### 공격 분류

| 항목 | 구분 |
|---|---|
| **스니핑 / DoS** | 도청·패킷 엿보기 / 자원 고갈·서비스 마비(SYN·UDP Flooding, Teardrop) |
| **ICMP Flooding** | **회선(대역폭)** 소진 |
| **SYN · ACK · DNS Query Flooding** | **서버 자원** 소진 |
| **Nimda / Stuxnet** | 파일·메일을 통한 확산 / **산업제어시스템(ICS)** 공격 |

### 탐지 도구·룰

| 항목 | 내용 |
|---|---|
| **Proexp (Process Explorer)** | 프로세스·핸들 확인 — 패킷 도구가 아니다 (패킷은 Wireshark) |
| **ModSecurity** | HTTP 요청 방화벽(WAF). `SecRuleEngine DetectionOnly`(탐지만) / `SecAuditEngine RelevantOnly`(관련 감사 로그) |
| Snort 위치 옵션 | `offset` 시작 / `depth` 종료 / `distance` 다음 검색 시작까지 거리 / `within` 다음 검색 허용 범위 |

**함정 포인트**
- **IPv6 접두사(FE/FC/FF/2)와 사설 IPv4 대역**이 반복 오답 1·2위다. 숫자만 따로 인출한다.
- **ICMP는 회선, SYN·DNS는 서버 자원.** 소진 대상이 다르다.
- **802.1X(포트 인증) ↔ 802.11i(보안 표준)** 를 바꿔 낸다. 숫자가 비슷해 헷갈린다.
- Wireshark는 패킷, Proexp는 프로세스다. 도구의 관찰 대상을 바꿔 낸다.

📍 [2과목 네트워크보안] 1. 네트워크 일반 > 네트워크 개념 이해 / 2. 네트워크 기반 공격기술의 이해 및 대응 / 3. 네트워크 보안 기술 > 네트워크 보안기술 및 응용
