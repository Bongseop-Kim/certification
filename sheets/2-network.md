...
| 서브넷 호스트 수 | 2⁶ = 64 − 네트워크 주소 1 − 브로드캐스트 1 = **62** |
...
| **SMTP 명령 흐름** | **HELO/EHLO → MAIL FROM → RCPT TO → DATA**. HELO 일반 세션 / **EHLO 확장(ESMTP)** |

### 구성·접근통제

| 항목 | 내용 |
|---|---|
| **스크린드 서브넷** | DMZ 구성 + 보안성 가장 높음 + 구축·관리 복잡 |
| **NAC** | 접속하려는 **단말을 검사**해 허가된 단말만 네트워크에 들여보냄 |
...
| 스캔 응답 | **열린 포트 = 무응답 / 닫힌 포트 = RST** (FIN·NULL·XMAS) |
...
| 항목 | 내용 |
|---|---|
| **Wireshark** | 네트워크 패킷을 캡처해서 **내용을 분석**하는 도구 (GUI) |
| **Proexp (Process Explorer)** | 프로세스·핸들 확인 — 패킷 도구가 아니다 |
